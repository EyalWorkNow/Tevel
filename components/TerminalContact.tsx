...
const TerminalContact: React.FC<TerminalContactProps> = ({ t, isRtl }) => {
    ...
    const handleSubmit = (e: React.MouseEvent) => {
        ...
        const { name, email, org } = formState;

        // Construct email body
        const subject = `New Lead: ${name} - ${org}`;
        const body = `
      Name: ${name}
      Organization: ${org}
      Email: ${email}
      
      [Sent from Tevel Landing Page]
    `;

        // Open mail client in new tab/window to ensure it triggers
        setTimeout(() => {
            window.open(`mailto:eyalatiyawork@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`, '_blank');
            ...
            <div className="relative bg-[#050608] border border-white/10 rounded-xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-sm group">
                ...
                <div className="p-6 md:p-12 relative">
                    ...
                    <div className="relative z-10 flex flex-col gap-8">
                        ...
                        {/* Form Grid - Wrapped in form for Enter key support, but avoiding default submit */}
                        <form className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${isRtl ? 'rtl' : 'ltr'}`} dir={isRtl ? 'rtl' : 'ltr'}>
                        ...
                        {/* Submit Button */}
                        <button onClick={handleSubmit} className="relative group w-full mt-6 overflow-hidden rounded-lg outline-none focus:ring-2 focus:ring-emerald-500/50">
